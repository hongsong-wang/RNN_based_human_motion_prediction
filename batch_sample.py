...
class generate_train_data(Dataset):
    def __init__(self, data_set, source_seq_len, target_seq_len, seq_smooth=False, sample_start=16):
    ...
    def get_dic_action2index(self):
        actions = sorted(list(set([item[1] for item in list(self._data_set.keys())])) )
    ...
    def __getitem__(self, index):
        ...
        if self._seq_smooth and random.random() > 0.5:
            sigma = random.random()*2
            data_sel = gaussian_filter1d(data_sel, sigma, axis=-1)
